...
async def on_ready():
    print(bot.user.name)
    print("Connecting...")
    await bot.change_presence(status=",help | ,sim | Bot được phát triển bởi Kido, dựa trên ý tưởng của Thịnh. Have fun!", activity=discord.activity(name=watching, type=discord.ActivityType.watching, status=discord.Status.idle))
